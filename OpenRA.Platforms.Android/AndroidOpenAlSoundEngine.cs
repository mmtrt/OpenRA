 ...
 * Unofficial OpenRA Android port — OpenAL-soft via libopenal.so.
...
{
	sealed class AndroidOpenAlSoundEngine : ISoundEngine
	{
		...
		sealed class PoolSlot
		{
			...
			public AndroidOpenAlSoundSource SoundSource;
			public AndroidOpenAlSound Sound;
		}
		...
		public AndroidOpenAlSoundEngine(string deviceName)
		{
			if (deviceName != null)
				System.Diagnostics.Debug.WriteLine("Using sound device `{0}`", deviceName);
			else
				System.Diagnostics.Debug.WriteLine("Using default sound device");

			device = ALC10.alcOpenDevice(deviceName);
			if (device == IntPtr.Zero)
			{
				System.Diagnostics.Debug.WriteLine("Failed to open device. Falling back to default");
		...
		{
			return new AndroidOpenAlSoundSource(data, data.Length, channels, sampleBits, sampleRate);
		}
		...
		public ISound Play2D(ISoundSource soundSource, bool loop, bool relative, WPos pos, float volume, bool attenuateVolume)
		{
			...
			var alSoundSource = (AndroidOpenAlSoundSource)soundSource;
			...
			slot.Sound = new AndroidOpenAlSound(source, loop, relative, pos, volume * atten, alSoundSource.SampleRate, alSoundSource.Buffer);
			return slot.Sound;
		}
		...
		public ISound Play2DStream(Stream stream, int channels, int sampleBits, int sampleRate, bool loop, bool relative, WPos pos, float volume)
		{
			...
			slot.IsRelative = relative;
			slot.SoundSource = null;
			slot.Sound = new AndroidOpenAlAsyncLoadSound(source, loop, relative, pos, volume, channels, sampleBits, sampleRate, stream);
			return slot.Sound;
		}
		...
		public void PauseSound(ISound sound, bool paused)
		{
			...
			var source = ((AndroidOpenAlSound)sound).Source;
		...
		public void SetSoundVolume(float volume, ISound music, ISound video)
		{
			var sounds = sourcePool.Keys.Where(key =>
			{
				...
				return (state == AL10.AL_PLAYING || state == AL10.AL_PAUSED) &&
					   (music == null || key != ((AndroidOpenAlSound)music).Source) &&
					   (video == null || key != ((AndroidOpenAlSound)video).Source);
			});
		...
		{
			((AndroidOpenAlSound)sound)?.SetLooping(looping);
		}

		public void SetSoundPosition(ISound sound, WPos position)
		{
			((AndroidOpenAlSound)sound)?.SetPosition(position);
		}

		~AndroidOpenAlSoundEngine()
		{
			Dispose(false);
		}
	...
	sealed class AndroidOpenAlSoundSource : ISoundSource
	{
		uint buffer;
		bool disposed;
		...
		public AndroidOpenAlSoundSource(byte[] data, int byteCount, int channels, int sampleBits, int sampleRate)
		{
			SampleRate = sampleRate;
			AL10.alGenBuffers(1, out buffer);
			AL10.alBufferData(buffer, AndroidOpenAlSoundEngine.MakeALFormat(channels, sampleBits), data, byteCount, sampleRate);
		}
	...
	class AndroidOpenAlSound : ISound
	{
		...
		public AndroidOpenAlSound(uint source, bool looping, bool relative, WPos pos, float volume, int sampleRate, uint buffer)
		...
		protected AndroidOpenAlSound(uint source, bool looping, bool relative, WPos pos, float volume, int sampleRate)
		{
			Source = source;
			SampleRate = sampleRate;
			Volume = volume;
	...
	sealed class AndroidOpenAlAsyncLoadSound : AndroidOpenAlSound
	{
		...
		public AndroidOpenAlAsyncLoadSound(uint source, bool looping, bool relative, WPos pos, float volume, int channels, int sampleBits, int sampleRate, Stream stream)
			: base(source, looping, relative, pos, volume, sampleRate)
		{
			...
			var silentSource = new AndroidOpenAlSoundSource(SilentData, SilentData.Length, channels, sampleBits, sampleRate);
			...
			playTask = Task.Run(async () =>
			{
				...
				using (var soundSource = new AndroidOpenAlSoundSource(data, dataLength, channels, sampleBits, sampleRate))
				{
