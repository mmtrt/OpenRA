...
namespace OpenRA.Mods.Common.Widgets.Logic
{
	public class DisplaySettingsLogic : ChromeLogic
	{
		...
		public static void ShowUIScaleDropdown(DropDownButtonWidget dropdown, GraphicSettings graphicSettings)
		{
			...
			// Always raise the ceiling on Android so 1.80–2.5 remain selectable AND loadable.
			var isAndroid =
#if ANDROID
				true;
#else
			...
			if (isAndroid)
			{
				if (Game.Renderer.NativeResolution.Width >= 1920 && Game.Renderer.NativeResolution.Height >= 1080)
					...
					maxScale = Math.Max(maxScale, 1.80f);
			}

			var validScales = new[] { 1f, 1.25f, 1.5f, 1.75f, 1.80f, 2f, 2.25f, 2.5f }.Where(x => x <= maxScale + 0.001f);
